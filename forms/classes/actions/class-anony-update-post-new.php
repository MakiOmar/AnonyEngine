...
if ( ! class_exists( 'ANONY_Update_Post_New' ) ) {
	 ...
	 */
	class ANONY_Update_Post_New extends ANONY_Actions_Base {
		...
		public function __construct( $validated_data, $action_data, $form ) {
			...
			if ( ! isset( $action_data['post_data'] ) ) {
				...
				error_log( 'Class ANONY_Update_Post_New : Missing post_data parameter' );
				//phpcs:enable
				return;
			}
			...
			if ( ! empty( $diff ) ) {
				//phpcs:disable WordPress.PHP.DevelopmentFunctions.error_log_error_log
				error_log( 'Class ANONY_Update_Post_New : post_data parameter missing required keys' );
				//phpcs:enable
				return;
			}
