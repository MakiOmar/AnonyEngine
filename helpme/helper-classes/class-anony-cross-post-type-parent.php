	...
	class ANONY_Cross_Post_Type_Parent {
		...
		public function parent_meta_boxes_cb( $post ) {
			...
			$pages = wp_dropdown_pages(
				array(
					'post_type'        => esc_sql( $this->parent ),
					'selected'         => esc_html( $post->post_parent ),
					'name'             => 'parent_id',
					'show_option_none' => esc_html__( '(no parent)', 'anonyengine' ),
					'sort_column'      => 'menu_order, post_title',
					'echo'             => 0,
				)
			);
			if ( ! empty( $pages ) ) {
				echo $pages; // phpcs:disable WordPress.Security.EscapeOutput.OutputNotEscaped
			} else {
		...
		public function anony_cross_parent_permalink_cb( $permalink, $post, $leavename ) {
			...
			if ( ! in_array( $post->post_type, array_keys( $post_parents ), true ) || empty( $permalink ) || in_array( $post->post_status, array( 'draft', 'pending', 'auto-draft' ), true ) ) {
				return $permalink;
			}
