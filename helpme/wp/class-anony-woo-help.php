...
if ( ! class_exists( 'ANONY_WOO_HELP' ) ) {
	class ANONY_WOO_HELP extends ANONY_HELP {
		...
		public static function CouponFormInOrderDetails() {
			...
			add_action( 'woocommerce_before_checkout_form', array( self, 'hideCheckoutCouponForm' ), 5 );

			// Add a custom coupon field before checkout payment section
			add_action( 'woocommerce_review_order_before_payment', array( self, 'checkoutCouponFormCustom' ) );

			// jQuery code
			add_action( 'wp_footer', array( self, 'customCheckoutJqueryScript' ) );
		}
		...
		static function showOnlyLowestVariationPrice() {
			add_filter( 'woocommerce_variable_sale_price_html', array( self, 'lowestVariationPriceFormat' ), 10, 2 );
			add_filter( 'woocommerce_variable_price_html', array( self, 'lowestVariationPriceFormat' ), 10, 2 );
		}
		...
		static function showOnlyHighestVariationPrice( $price, $product ) {
			add_filter( 'woocommerce_variable_sale_price_html', array( self, 'highesttVariationPriceFormat' ), 10, 2 );
			add_filter( 'woocommerce_variable_price_html', array( self, 'highesttVariationPriceFormat' ), 10, 2 );
		}
		...
		static function disableCustomerRepeatPurchase() {
			add_filter( 'woocommerce_is_purchasable', array( self, 'disableRepeatPurchase' ), 10, 2 );
			add_action( 'woocommerce_single_product_summary', array( self, 'purchaseDisabledMessage' ), 31 );
		}
		...
		static function removingCustomerDetailsInEmails() {
			add_action( 'woocommerce_email_customer_details', array( self, 'removingCustomerDetailsInEmailsCb' ), 5, 4 );
			add_action( 'woocommerce_email_after_order_table', array( self, 'removingCustomerDetailsInEmailsCb' ), 10, 2 );
		}
		...
		static function checkoutRemoveItems() {
			add_filter( 'woocommerce_cart_item_name', array( self, 'checkoutRemoveItemsCb' ), 10, 3 );
		}
		/**
