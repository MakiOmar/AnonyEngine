...
function anony_share_by_email(){
    if(!isset($_POST['anony_share_email']) || empty($_POST['anony_share_email'])) return;
     ...
     if(!is_email(sanitize_email($_POST['anony_share_email']))){
        ...
        wp_send_json(
            [
                'error'  => 'no_email',
                'report' => esc_html__('Please make sure you have entered a valid email address',ANOE_TEXTDOM ),
            ]
        ); die();
     }
    ...
    $subject = esc_html__('anony Real Estate',ANOE_TEXTDOM );
    ...
    if(!$email_sent){
        ...
        wp_send_json(
            [
                'error'  => 'not_sent',
                'report' => esc_html__('Email can\'t be sent',ANOE_TEXTDOM ),
            ]
        ); die();
    }
    ...
    wp_send_json(
            [
                'msg'  => 'email_sent',
                'report' => esc_html__('Email has been be sent successfully',ANOE_TEXTDOM ),
            ]
        ); die();
}